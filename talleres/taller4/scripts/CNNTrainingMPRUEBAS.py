# -*- coding: utf-8 -*-
"""
Created on Mon Jul 19 20:37:06 2021
    ...
    def trainCNN(self, trainingData, labels, nFolds = 10, saveBestWeights = True):
        ...
        if not self.model: #Check if themodel is empty
            ...
            for trainIndex, testIndex in kf.split(trainingData):
                ...
                accur.append(actualSscore[1]*100)
...
for s in subjects:
    ...
    **********************************************************************
    ...
    title = f"EEG without filter - subjetc {s}"
    plotEEG(signal = rawEEG, sujeto = s,
            trial = 3, blanco = 1, window = [0,4], fm = 256.0, save = False, title = title)
    ...
    title = f"EEG filtered - subjetc {s}"
    
    plotEEG(eegfiltrado, sujeto = s,
            trial = 3, blanco = 1, window = [0,4], fm = 256.0, save = False,
            title = title)
    ...
    **********************************************************************
    ...
    #Make a CNNTrainingModule object in order to use the data's Magnitude Features
    magnitudCNN = CNNTrainingModule(rawEEG = rawEEG, subject = s,
                                    PRE_PROCES_PARAMS = PRE_PROCES_PARAMS,
                                    FFT_PARAMS = FFT_PARAMS,
                                    CNN_PARAMS = CNN_PARAMS,
                                    modelName = f"CNN_UsingMagnitudFeatures_Subject{s}")
    ...
    **********************************************************************
    ...
    plotSpectrum(magnitudFeatures, resolution, 12, s, 7, frecStimulus,
                  startFrecGraph = FFT_PARAMS['start_frequency'],
                  save = False, title = "", folder = "figs")
    ...
    **************************************
    """
    """Make a CNNTrainingModule object in order to use the data's Magnitude and data's Complex Features"""
    complexCNN = CNNTrainingModule(rawEEG = rawEEG, subject = s,
                                    PRE_PROCES_PARAMS = PRE_PROCES_PARAMS,
                                    FFT_PARAMS = FFT_PARAMS,
                                    CNN_PARAMS = CNN_PARAMS,
                                    modelName = f"CNN_UsingComplexFeatures_Subject{s}")
    ...
    Accuracies.setdefault(f's{s}', accur)
    
f = 0
#plot = 211
k = list(frecStimulus[:10])
#fig, axes = plt.subplots(5, 2, figsize=(24, 20), gridspec_kw = dict(hspace=0.45, wspace=0.2))
...
for p in k:
    ...
    for i in subjects:
        ...
        acus2.append(Accuracies[f's{i}'][f])
        if len(acus2) == 2:
            #plt.subplot(plot)
            plt.bar(x = strings, height = acus2, align = 'center', width = 0.4, color = 'purple')
            plt.xlabel("Frecuencia(Hz)")
            plt.ylabel('Accuracy (%)')
            plt.title(f'Frecuencia del estímulo {p} Hz')
            f = f+1
            plt.show()
            #plot = plot + 1
...
'''
for i in frecStimulus[:10]:
    strings.append(str(i))

fig, axes = plt.subplots(5, 2, figsize=(8, 6), gridspec_kw = dict(hspace=0.4, wspace=0.2))

plt.subplot(211)
plt.bar(x = strings, height = accur[:10], align = 'center', width = 0.4, color = 'red')
plt.xlabel("Frecuencia(Hz)")
plt.ylabel('Accuracy (%)')
plt.title(f'Accuracy para Magnitud Sujeto {subjects[0]}')

plt.subplot(212)
plt.bar(x = strings, height = accur[10:], align = 'center', width = 0.4, color = 'red')
plt.xlabel("Frecuencia(Hz)")
plt.ylabel('Accuracy (%)')
plt.title(f'Accuracy para Complejo Sujeto {subjects[0]}')
plt.show()
'''