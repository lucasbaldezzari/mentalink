...
def plotEEG(signal, sujeto = 1, trial = 1, blanco = 1,
    ...
    fig, axes = plt.subplots(1, 1, figsize=(10, 8), gridspec_kw = dict(hspace=0.45, wspace=0.2))
    ...
    for canal in range(channelsNums):
        ...
        plt.plot(t, signalScale, color = "#e37165")
        plt.xlabel('Tiempo [seg]', fontsize=16) 
        plt.ylabel('Amplitud [uV]', fontsize=16)
        plt.title(f'Sujeto {sujeto} - Blanco {blanco} - Canal {canal + 1}', fontsize=22)
        #plt.yaxis.grid(True)
        break
...
def plotSpectrum(espectroSujeto, resol, blancos, sujeto, canal, frecStimulus,
    ...
    fig, plots = plt.subplots(1, 1, figsize=(10, 8), gridspec_kw=dict(hspace=0.45, wspace=0.3))
   # plots = plots.reshape(-1)
    ...
    for blanco in range(1):
        fft_axis = np.arange(espectroSujeto.shape[0]) * resol
        fft_y = np.mean(np.squeeze(espectroSujeto[:, canal, blanco, :, :]), 
                                          axis=1)
        plt.plot(fft_axis, fft_y, color = "#403e7d")
        plt.xlabel('Frecuencia [Hz]')
        plt.ylabel('Amplitud [uV]')
        plt.title('Estímulo 9.25 Hz del sujeto 8')
        #plots[blanco].xaxis.grid(True)
        plt.axvline(x = frecStimulus[blanco], ymin = 0., ymax = max(fft_axis),
                             label = "Frec. Estímulo",
    ...
    for i in range(len(a)): #codigo para detecta maximos sobre umbrales
        if abs(a[i+2]) ==1:
            break
        if abs(a[i+1]) > abs(a[i]) and abs(a[i+1]) >= abs(a[i+2]):
            picosm.append(a[i+1])
            picosf.append(fft_axis[i+1])
    
    print(f"hay un máximo en {picosf[picosm.index(max(picosm))]}")
